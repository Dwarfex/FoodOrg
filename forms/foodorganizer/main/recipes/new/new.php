...
$sePage->setTitle('FO - Rezept erstellen');


//get new recipe id
$currentUserIdString = (string) $seUser->isAuthCurrentUser();
$currentUserName  = (string) $seUser->getUserLogin($seUser->isAuthCurrentUser());
...
if (!$allowEdit) navigate(\Package\FoodOrganizer\FOHelpers::getFOBaseDirectory() . 'recipes/all/');

if($allowEdit && !isset($_POST['recipeID'])){
    $recipeController = new \Package\FoodOrganizer\Controller\recipeController();
    $recipe = $recipeController->getNewRecipe($currentUserIdString, $currentUserName);
...
if ((count($_POST) > 0) && $allowEdit) {
    ...
    if (se::isClicked('save')) {
        ...
        //todo : save here
       $result = $recipeController->saveNew($recipe);

        /* @var $result \MongoDB\UpdateResult */

        $recipe = $recipeController->find(['_id'=>$result->getUpsertedId()]);


        foreach ($recipe as $insertedRecipe){


            /* @var \Package\FoodOrganizer\Models\Recipe\Recipe $insertedRecipe */

            navigate(\Package\FoodOrganizer\FOHelpers::getFOBaseDirectory() . '/recipes/details/' . $insertedRecipe->getRezeptId() . '/' . $insertedRecipe->getRezeptDatum());
...
<div class="panel panel-default">
   ...
   ' . beginFormular('editRecipe', \Package\FoodOrganizer\FOHelpers::getFOBaseDirectory() . '/recipes/new" class="form-horizontal" role="form" data-toggle="validator') . '
