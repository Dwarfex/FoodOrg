...
$sitehtml='<div class="row"><div class="col-md-12"><h1>Meine Rezepte</h1></div></div>
...
$result = $recipeController->findByUserId($currentUserIdString);
...
$i=0;
if(count($result) == 0){
    $sitehtml.='Du hast leider noch keine Rezepte'.br();
}else{
...
<a href="'.\Package\FoodOrganizer\FOHelpers::getFOBaseDirectory().'/recipes/details/'.$recipe->getRezeptId().'/'.$recipe->getRezeptDatum().'" ><img class="img-responsive img-rounded rezeptVorschauBild" src="'.$recipeImage->getSmallImage().'" alt=""></a>';
    $sitehtml.='<br/>User:<a href="'.\Package\FoodOrganizer\FOHelpers::getFOBaseDirectory().'/user/'.$recipe->getRezeptUserId().'/'.$recipe->getRezeptUserName().'">'.$recipe->getRezeptUserName().'</a><br/>Bewertung:';

//    $emtpyStars = 5 - $starsOutOfFive;

    $sitehtml.=\Package\FoodOrganizer\FOHelpers::generateStarsFromRating($recipe->getRezeptVotes()->getAverage()).'<br/>';
